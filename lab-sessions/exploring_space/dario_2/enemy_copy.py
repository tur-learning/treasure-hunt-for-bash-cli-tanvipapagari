...
class Enemy:
    ...
    def move_around_circle(self, speed, start_pos):
        # Update the angle
        self.angle += speed % (2*math.pi)
        #self.angle = 0.
        # Calculate the new x and y position based on the angle and radius
        pos_A = pygame.math.Vector2(start_pos)
        self.x = int(self.center_x + self.radius * math.cos(math.radians(self.angle)) + pos_A.x)
        self.y = int(self.center_y + self.radius * math.sin(math.radians(self.angle)) + pos_A.y)
    
    def draw(self, surface, x1, y1):
        ...
        self.move_around_circle(1, (- x1, - y1))
        ...
        print(self.x)
        surface.blit(scaled_surface, (self.x, self.y))
